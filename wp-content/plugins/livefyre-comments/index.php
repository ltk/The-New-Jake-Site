...
function livefyre_request($path, $upload=false, $data=null, $type='POST', $headers=null) {
    ...
    if ($upload) {
        ...
        $result = $request->request( $path, array( 'method' => $type, 'body' => $data, 'headers' => $headers, 'timeout'=>120) );
    }
    unset($request);
    if (is_wp_error($result)) {
...
function livefyre_activate() {
    ...
    livefyre_reset_caches();
    $v3_site_id = get_option('livefyre_site_id', false);
    if ($v3_site_id !== false) {
        // if you are rolling back to V2, you need some help here to get
        // values stored under the right option names.
        update_option('livefyre_blogname', $v3_site_id);
        update_option('livefyre_secret', get_option('livefyre_site_key', false));
    }
